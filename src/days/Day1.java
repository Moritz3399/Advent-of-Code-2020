...
public class Day1 {
    ...
    private Day1(String[] input) {
        ...
        System.out.printf("First Answer: %s with %s%n",Arrays.stream(partA).reduce(1, (a, b) -> a*b),Arrays.toString(partA));

        int[] partB = findEntries(3,0, new int[0], -1);
        System.out.printf("Second Answer: %s with %s%n",Arrays.stream(partB).reduce(1, (a, b) -> a*b),Arrays.toString(partB));
    }
    ...
    private int[] findEntries(int howManyEntriesToFind, int level, int[] previousLevels, int previousIndex) {
        ...
        for (int i = previousIndex + 1; i < expenses.length; i++) {
